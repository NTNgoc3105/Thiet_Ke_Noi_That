...
function setActiveNav() {
  qsa(".nav-link").forEach((a) =>
    a.classList.toggle("active", a.dataset.link === pageName),
  );
}
...
function updateCartBadge() {
  qsa(".cart-badge").forEach((b) => (b.textContent = cartCount()));
}
...
function shippingCost(subtotal) {
  return subtotal >= 3750000 ? 0 : 150000;
}
...
function openCart() {
  ...
  o.classList.remove("pointer-events-none", "opacity-0");
  o.classList.add("pointer-events-auto", "opacity-100");
...
function closeCart() {
  ...
  o.classList.add("pointer-events-none", "opacity-0");
  o.classList.remove("pointer-events-auto", "opacity-100");
  document.body.classList.remove("overflow-hidden");
}
function openSearch() {
  const m = qs("#search-modal");
  if (!m) return;
...
function renderDrawer() {
  ...
  box.innerHTML = cart.length
    ? cart
        .map(
          (i) =>
            `<div class="flex gap-4 rounded-xl bg-[#F9F1E7] p-3"><img class="h-20 w-20 rounded object-cover" src="${i.image}" alt="${i.name}"><div class="min-w-0 flex-1"><div class="flex justify-between gap-3"><div><div class="truncate font-semibold">${i.name}</div><div class="mt-1 text-xs text-black/45">${i.color} · ${i.size}</div></div><button class="text-xs text-black/40 hover:text-red-600" data-remove-cart="${i.key}">Remove</button></div><div class="mt-3 flex items-center justify-between"><div class="flex items-center overflow-hidden rounded border border-black/10 bg-white"><button class="h-8 w-8" data-cart-delta="${i.key}" data-delta="-1">−</button><span class="grid h-8 w-8 place-items-center text-sm">${i.qty}</span><button class="h-8 w-8" data-cart-delta="${i.key}" data-delta="1">+</button></div><strong class="text-sm">${money(i.price * i.qty)}</strong></div></div></div>`,
        )
        .join("")
    : `<div class="grid h-full place-items-center text-center"><div><div class="text-3xl">🛒</div><p class="mt-3 font-medium">Your cart is empty</p><a class="mt-4 inline-block btn-primary" href="shop.html">Start shopping</a></div></div>`;
  qs("#drawer-subtotal") &&
    (qs("#drawer-subtotal").textContent = money(cartSubtotal()));
  const free = qs("#drawer-free");
  if (free) {
    const need = Math.max(0, 3750000 - cartSubtotal());
    free.classList.toggle("hidden", !cart.length || need <= 0);
    free.textContent =
      need > 0
        ? `Buy ${money(need)} more and get free shipping.`
...
function renderShop() {
  ...
  qs("#category-filters") &&
    (qs("#category-filters").innerHTML = categories
      .map(
        (c) =>
          `<button class="filter-chip ${shopState.category === c ? "selected" : ""}" data-category="${c}">${c}</button>`,
      )
      .join(""));
  let data = products
    .filter(
      (p) => shopState.category === "All" || p.category === shopState.category,
    )
    .filter((p) => p.price <= shopState.maxPrice)
    .filter(
      (p) =>
        !shopState.search ||
        `${p.name} ${p.description}`
          .toLowerCase()
          .includes(shopState.search.toLowerCase()),
    );
  if (shopState.sort === "price-asc") data.sort((a, b) => a.price - b.price);
  if (shopState.sort === "price-desc") data.sort((a, b) => b.price - a.price);
  if (shopState.sort === "name")
    data.sort((a, b) => a.name.localeCompare(b.name));
  const total = data.length;
  const pages = Math.max(1, Math.ceil(total / shopState.show));
  shopState.page = Math.min(shopState.page, pages);
  const start = (shopState.page - 1) * shopState.show;
  const visible = data.slice(start, start + shopState.show);
  grid.className =
    shopState.view === "list"
      ? "grid gap-5"
  ...
  grid.innerHTML = visible
    .map((p) =>
      shopState.view === "grid"
        ? productCard(p)
        : `<article class="flex gap-5 border-b border-black/10 pb-5"><img class="h-40 w-40 object-cover bg-[#F4F5F7]" src="${p.image}" alt="${p.name}"><div><a href="product.html?id=${p.id}" class="text-xl font-semibold">${p.name}</a><p class="mt-2 text-sm text-black/50">${p.description}</p><div class="mt-3 font-semibold">${money(p.price)}</div><div class="mt-4 flex flex-wrap gap-2"><button class="btn-outline" data-quick-add="${p.id}">Add to cart</button><button class="btn-outline" data-compare="${p.id}">Compare</button></div></div></article>`,
    )
    .join("");
  qs("#results-label") &&
    (qs("#results-label").textContent =
      `Showing ${total ? start + 1 : 0}–${Math.min(start + visible.length, total)} of ${total} results`);
  const pag = qs("#pagination");
  if (pag)
    pag.innerHTML = Array.from(
      { length: pages },
      (_, i) =>
        `<button class="page-btn ${shopState.page === i + 1 ? "selected" : ""}" data-page="${i + 1}">${i + 1}</button>`,
    ).join("");
}
...
function renderProduct() {
  ...
  let size = p.sizes[0],
    color = p.colors[0];
  qsa("[data-size]", root).forEach((b) =>
    b.addEventListener("click", () => {
      size = b.dataset.size;
      qsa("[data-size]", root).forEach((x) => x.classList.remove("selected"));
      b.classList.add("selected");
    }),
  );
  qsa("[data-color]", root).forEach((b) =>
    b.addEventListener("click", () => {
      color = b.dataset.color;
      qsa("[data-color]", root).forEach((x) => x.classList.remove("selected"));
      b.classList.add("selected");
    }),
  );
  qsa("[data-gallery]", root).forEach((b) =>
    b.addEventListener("click", () => {
      qs("#main-product-image").src = b.dataset.gallery;
      qsa("[data-gallery]", root).forEach((x) =>
        x.classList.remove("border-[#B88E2F]"),
      );
      b.classList.add("border-[#B88E2F]");
    }),
  );
  qsa("[data-product-qty]", root).forEach((b) =>
    b.addEventListener("click", () => {
      qty = Math.max(1, qty + Number(b.dataset.productQty));
      qs("#product-qty").textContent = qty;
    }),
  );
  qs("#product-add", root)?.addEventListener("click", () =>
...
function renderCompare() {
  ...
  const items = ids
    ...
    .filter(Boolean);
  const root = qs("#compare-root");
  if (!root) return;
  const rows = [
    ["General", "Category", ...items.map((x) => x.category)],
    ["General", "Brand", ...items.map((x) => x.brand)],
    ["Dimensions", "Size", ...items.map((x) => x.sizes.join(" · "))],
    ["Dimensions", "Color options", ...items.map((x) => x.colors.join(" · "))],
    ["Warranty", "Warranty", ...items.map(() => "2 years")],
    ["Warranty", "Price", ...items.map((x) => money(x.price))],
  ];
  root.innerHTML = items.length
    ? `<div class="overflow-x-auto"><table class="min-w-[860px] w-full border-collapse"><thead><tr><th class="w-[180px] bg-[#F9F1E7] p-4 text-left">Specification</th>${items.map((p) => `<th class="min-w-[220px] p-4 align-top"><img class="aspect-square w-full object-cover bg-[#F4F5F7]" src="${p.image}" alt="${p.name}"><div class="mt-3 text-lg font-semibold">${p.name}</div><button class="mt-2 text-xs text-red-600" data-remove-compare="${p.id}">Remove</button></th>`).join("")}</tr></thead><tbody>${rows
        .map((row, ri) => {
          const vals = row.slice(2);
          const diff = new Set(vals).size > 1;
          return `<tr class="border-t"><td class="bg-[#F9F1E7] p-4 font-medium">${row[1]}</td>${vals.map((v) => `<td class="p-4 text-center ${diff ? "bg-[#FFF8E9]" : ""} ${ri === 5 ? "font-semibold" : ""}">${v}${diff ? '<span class="ml-2 inline-block h-1.5 w-1.5 rounded-full bg-[#B88E2F]" title="Different"></span>' : ""}</td>`).join("")}</tr>`;
        })
        .join("")}</tbody></table></div>`
    : `<div class="py-24 text-center"><div class="text-4xl">♡</div><h2 class="mt-3 text-2xl font-semibold">Nothing to compare yet</h2><p class="mt-2 text-black/50">Add products from the shop or detail page.</p><a class="mt-6 inline-flex btn-primary" href="shop.html">Browse products</a></div>`;
}
...
function renderCheckout() {
  ...
  if (!root) return;
  const saved = getJSON(STORE.shipping, {});
  const cart = getCart();
  const subtotal = cartSubtotal();
  const ship = shippingCost(subtotal);
...
function renderContact() {
  ...
  form.addEventListener("submit", (e) => {
    e.preventDefault();
    const message = qs("#contact-success");
    if (form.checkValidity()) {
      ...
      message.className = "rounded bg-red-50 p-3 text-sm text-red-700";
      message.classList.remove("hidden");
      form.reportValidity();
    }
  });
...
function bindGlobal() {
  document.addEventListener("click", (e) => {
    ...
    if (a) {
      ...
      if (ac === "cart") openCart();
      if (ac === "close-cart") closeCart();
      if (ac === "search") openSearch();
      if (ac === "close-search") closeSearch();
      if (ac === "account") openAccount();
      if (ac === "close-account") closeAccount();
      if (ac === "mobile-menu") qs("#mobile-menu")?.classList.toggle("hidden");
    }
    const q = e.target.closest("[data-quick-add]");
    if (q) {
      const p = products.find((x) => x.id === Number(q.dataset.quickAdd));
      if (p) addToCart(p, 1, p.sizes[0], p.colors[0]);
    }
    ...
    if (rd) {
      removeCartItem(rd.dataset.removeCart);
    }
    const cd = e.target.closest("[data-cart-delta]");
    if (cd) {
      updateCartItem(cd.dataset.cartDelta, Number(cd.dataset.delta));
    }
    const rc = e.target.closest("[data-remove-compare]");
    if (rc) {
      setJSON(
        STORE.compare,
        getJSON(STORE.compare, []).filter(
          (x) => x !== Number(rc.dataset.removeCompare),
        ),
      );
      renderCompare();
    }
    ...
    if (pbtn) {
      shopState.page = Number(pbtn.dataset.page);
      renderShop();
    }
    const cat = e.target.closest("[data-category]");
    if (cat) {
      shopState.category = cat.dataset.category;
      shopState.page = 1;
      renderShop();
    }
    const tab = e.target.closest("[data-tab]");
    if (tab) {
      const root = tab.closest("#product-detail");
      if (root) {
        qsa(".tab-link", root).forEach((x) => x.classList.remove("active"));
        tab.classList.add("active");
        qsa("[data-tab-panel]", root).forEach((x) =>
          x.classList.toggle("hidden", x.dataset.tabPanel !== tab.dataset.tab),
        );
      }
    }
    ...
    if (share) {
      navigator.clipboard?.writeText(location.href).then(() => {
        ...
        setTimeout(() => (share.textContent = "Share article"), 1400);
      });
    }
  });
  ...
  qs("#global-search")?.addEventListener("input", (e) => {
    ...
    const list = products
      ...
      .slice(0, 5);
    qs("#search-results").innerHTML = q
      ? list
          .map(
            (p) =>
              `<a href="product.html?id=${p.id}" class="flex items-center gap-4 rounded-lg p-2 hover:bg-black/5"><img src="${p.image}" class="h-12 w-12 object-cover" alt=""><div><div class="font-semibold">${p.name}</div><div class="text-xs text-black/45">${money(p.price)}</div></div></a>`,
          )
          .join("")
      : '<div class="py-8 text-sm text-black/40">Start typing to search the collection.</div>';
  });
  qsa('[data-form="subscribe"]').forEach((f) =>
    f.addEventListener("submit", (e) => {
      e.preventDefault();
      const p = f.querySelector("input");
      const err = qs('[data-error="subscribe"]');
      if (p.checkValidity()) {
        err.textContent = "Subscribed — welcome to Furniro.";
        err.className = "mt-2 text-xs text-emerald-600";
        f.reset();
      } else {
  ...
  qsa('[data-form="login"]').forEach((f) =>
    f.addEventListener("submit", (e) => {
      e.preventDefault();
      const err = qs('[data-error="login"]');
      if (f.checkValidity()) {
        err.textContent = "Demo login successful.";
        err.className = "text-xs text-emerald-600";
        setTimeout(closeAccount, 700);
      } else {
        err.textContent = "Please check your email and password.";
...
function bindShopControls() {
  qs("#shop-search")?.addEventListener("input", (e) => {
  ...
  qs("#shop-show")?.addEventListener("change", (e) => {
    shopState.show = Number(e.target.value);
    shopState.page = 1;
    renderShop();
  });
  qs("#shop-max-price")?.addEventListener("input", (e) => {
    shopState.maxPrice = Number(e.target.value);
    qs("#shop-max-label").textContent = money(shopState.maxPrice);
    shopState.page = 1;
    renderShop();
  });
  qs("#view-grid")?.addEventListener("click", () => {
    shopState.view = "grid";
    renderShop();
  });
  ...
  qs("#filter-open")?.addEventListener("click", () =>
    qs("#shop-filters")?.classList.toggle("hidden"),
  );
}
...
function bindCheckout() {
  qs("#checkout-form")?.addEventListener("submit", (e) => {
    ...
    const msg = qs("#checkout-message");
    if (form.checkValidity()) {
      if (qs('input[name="saveInfo"]')?.checked) {
        setJSON(
          STORE.shipping,
      ...
      msg.textContent =
        "Order placed successfully. This demo stops before real payment.";
  ...
  qs("#same-address")?.addEventListener("change", () => {
    if (qs("#same-address").checked) {
      ["city", "postal"].forEach((n) => {
        const i = qs(`[name="${n}"]`);
        if (i) i.value = savedValue(n);
      });
    }
  });
  function savedValue(n) {
    return getJSON(STORE.shipping, {})[n] || "";
  }
  qs("#apply-discount")?.addEventListener("click", () => {
    const code = qs("#discount-code").value.trim().toUpperCase();
    const note = qs("#discount-note");
    note.textContent =
      code === "FURNIRO10"
        ? "10% discount applied to your order."
        : "Invalid code. Try FURNIRO10.";
    note.className =
      code === "FURNIRO10"
...
(async () => {
  ...
  if (pageName === "blog") {
    if (new URLSearchParams(location.search).has("post")) renderBlogDetail();
    else {
      renderBlog();
      qs("#blog-search")?.addEventListener("input", renderBlog);
      qs("#blog-filter")?.addEventListener("change", renderBlog);
    }
  }
  if (pageName === "contact") {
    renderContact();
